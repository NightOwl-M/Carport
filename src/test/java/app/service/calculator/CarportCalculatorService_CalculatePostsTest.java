...
public class CarportCalculatorService_CalculatePostsTest {
    ...
    @Test
    void calculatePost() throws DatabaseException {
        //Arrange
        Order order = new Order(3, 3, 510, 630, "Plasttrapezplader", "Ønsker det i sort", "Tak for snakken", 3, 25000, Timestamp.valueOf("2025-05-08 12:30:15"));
        CarportCalculatorService calc = new CarportCalculatorService(630, 510, connectionPool);
        List<Component> expectedOrderComponents = new ArrayList<>();


        /* Beregning af expedtedOrderComponents
        //Stolper findes kun i 1 længde, 300 cm

        Først findes længden imellem de 2 yderste stolper ved at fratrække udhænget til begge sider:
        630 cm – 30 cm – 100 cm = 500 cm
        Nu kan den resterende længde inddeles i moduler, som består af 1 stolpe og det efterfølgende tomrum til næste stolpe:
        500 cm / 310 cm = 1,6 ~ 2 moduler
        Der rundes op til nærmeste heltal da vi ikke kan have 1.6 moduler og vi vil altid hellere have en stolpe for meget end én for lidt. Det giver os altså 2 stolper, dog mangler der at blive taget højde for den sidste stolpe, som ikke skal have et efterfølgende modul:
        2 + 1 = 3 stolper
        Til slut ganges med 2, da carporten har 2 sider med stolper
        3 * 2 = 6 stolper i alt

         */
        Material post = new Material(5, "97x97 mm. trykimp. Stolpe", "stk", 75);
        MaterialVariant postVariant300 = new MaterialVariant(1, 300, post);
        Component componentPostVariant300 = new Component(3, 6, "Stolper nedgraves 90 cm i jord", postVariant300);

        expectedOrderComponents.add(componentPostVariant300);


        //Act
        calc.calculatePosts(order);
        ...
        assertNotEquals(8, actualOrderComponents.get(0).getQuantity());
        assertNotEquals(360, actualOrderComponents.get(0).getMaterialVariant().getLength());
    }
}
